...
DCT 在压缩中的优势在于 **能量集中（Energy Compaction）** 特性。对于自然图像块，由于相邻像素间的高度相关性，变换后大部分能量将坍缩至低频率系数（左上角），而大量高频系数（右下角）的幅值趋近于零。这种分布规律将空间域中分散的像素能量重新排列，使后续的量化操作能够以较少的符号承载信号主体，并利用高频系数取整为零的统计特征提升熵编码效率。此外，变换本身为线性一一映射且完全可逆（在未引入舍入误差的前提下），这意味着变换环节自身不引入任何失真，其性质与量化环节形成鲜明互补。
